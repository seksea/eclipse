...
static void ScaleDamage(HitGroups hitgroup, Entity* enemy,
    ...
    if (enemy->nDT_CSPlayer__m_ArmorValue() > 0) {
        if (hitgroup == HitGroups::HITGROUP_HEAD) {
            if (enemy->nDT_CSPlayer__m_bHasHelmet())
...
static bool TraceToExit(Vector& end, Trace* enter_trace, Vector start,
                        Vector dir, Trace* exit_trace) {
    float distance = 0.0f;
    ...
    while (distance <= 90.0f) {
        ...
        auto point_contents = Interfaces::trace->getPointContents(
             end, 1174421515 /*MASK_SHOT_HULL | CONTENTS_HITBOX*/, nullptr);

        if (point_contents & 100679691 /*MASK_SHOT_HULL*/ &&
            !(point_contents & 1073741824 /*CONTENTS_HITBOX*/))
            continue;
        ...
        Interfaces::trace->traceRay(ray, 1174421507U /*MASK_SHOT*/, 0,
                                    exit_trace);

        if (exit_trace->startsolid &&
            exit_trace->surface.flags & 32768 /*SURF_HITBOX*/) {
            ray.Init(end, start);
...
static bool HandleBulletPenetration(CCSWeaponInfo* weaponInfo,
                                    Ragebot::fireBulletData& data) {
    surfacedata_t* enter_surface_data = Interfaces::physics->GetSurfaceData(
    ...
    surfacedata_t* exit_surface_data = Interfaces::physics->GetSurfaceData(
         trace_exit.surface.surfaceProps);
    ...
    if ((data.enter_trace.contents & 8 /*CONTENTS_GRATE*/) != 0 ||
    ...
    float v35 = (data.current_damage * final_damage_modifier) +
         v34 * 3.0f *
              fmaxf(0.0f, (3.0f / weaponInfo->GetPenetration()) * 1.25f);
...
static void TraceLine(Vector vecAbsStart, Vector vecAbsEnd,
                      unsigned int mask, Entity* ignore, Trace* ptr) {
    Ray ray;
...
static bool SimulateFireBullet(Entity* pWeapon, bool teamCheck,
                               Ragebot::fireBulletData& data) {
    Entity* localplayer = EntityCache::
         localPlayer;  //(C_BasePlayer*)
    ...
    while (data.penetrate_count > 0 && data.current_damage >= 1.0f) {
        data.trace_length_remaining =
             weaponInfo->GetRange() - data.trace_length;

        Vector retar = data.direction;
        retar *= data.trace_length_remaining;
        Vector end = data.src +
             retar /*data.direction * data.trace_length_remaining*/;

        // data.enter_trace
        TraceLine(data.src, end, 1174421507U /*MASK_SHOT*/, localplayer,
                  &data.enter_trace);
        ...
        Interfaces::trace->traceRay(ray, 1174421507U /*MASK_SHOT*/,
                                    &data.filter, &data.enter_trace);

        retar = data.direction;
        retar *= 40.f;
        TraceLine(data.src, end + retar /*data.direction * 40.f*/,
                  1174421507U /*MASK_SHOT*/, localplayer, &data.enter_trace);
        ...
        if (data.enter_trace.hitgroup <= HitGroups::HITGROUP_RIGHTLEG &&
            ...
            data.trace_length +=
                 data.enter_trace.fraction * data.trace_length_remaining;
            data.current_damage *= powf(weaponInfo->GetRangeModifier(),
                                        data.trace_length * 0.002f);

            Entity* player = (Entity*)data.enter_trace.m_pEntityHit;
            if (teamCheck && player->teammate()/*Entity::IsTeamMate(player, localplayer)*/)
                return false;

            ScaleDamage(data.enter_trace.hitgroup, player,
                        weaponInfo->GetWeaponArmorRatio(),
                        data.current_damage);
...
float Ragebot::GetDamage(const Vector& point, bool teamCheck,
                         fireBulletData& fData) {
    float damage = 0.f;
    Vector dst = point;
    Entity* localplayer = EntityCache::
         localPlayer;  //(C_BasePlayer*)
    ...
    Entity* activeWeapon = Interfaces::entityList->getClientEntity(
         EntityCache::localPlayer
              ->nDT_BaseCombatCharacter__m_hActiveWeapon() &
         0xfff);
...
Entity* clipRayToEntity(Vector start, Vector end, Trace* traceToPlayer,
                        Entity* pEntity) {
    Ray ray;
    ray.Init(start, end);
    // TraceFilter filter;
    ...
    Interfaces::trace->clipRayToEntity(ray, 1174421515, pEntity,
                                       traceToPlayer);
...
bool Ragebot::shouldWallbang(QAngle viewAngles, int minChance, int minDamage) {
    ...
    if (Entity* e = data.enter_trace.m_pEntityHit; !e ||
        e == EntityCache::localPlayer || e->teammate() ||
        e->nDT_BasePlayer__m_iHealth() == 0 ||
        e->nDT_CSPlayer__m_bGunGameImmunity())
        return false;

    if (data.current_damage < minDamage &&
        data.current_damage <
             data.enter_trace.m_pEntityHit->nDT_BasePlayer__m_iHealth())
        return false;
    ...
    float spread =
         RAD2DEG(activeWeapon->getInaccuracy() + activeWeapon->getSpread());
    for (int i = 0; i < 100 && hitChance < minChance; i++) {
        ...
        Entity* e =
             clipRayToEntity(EntityCache::localPlayer->eyepos(), endPos,
                             &traceToPlayer, data.enter_trace.m_pEntityHit);
