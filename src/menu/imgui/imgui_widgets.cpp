...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), false, style.FrameRounding);
    ...
    else if (*v)
    {
        ...
        ImColor topColor = ImColor(check_col);
        ImColor bottomColor = ImColor(topColor.Value.x * 0.4, topColor.Value.y * 0.4, topColor.Value.z * 0.4, 1.f);
        window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), topColor, topColor, bottomColor, bottomColor);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
            window->DrawList->AddText(ImVec2(bb.Max.x - 12, bb.Min.y - 1), text_col, popup_open ? "-" : "+"); //RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Down, 1.0f);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    window->DrawList->AddText(ImVec2(grab_bb.Max.x - (CalcTextSize(value_buf).x / 4) + 1, grab_bb.Min.y - (CalcTextSize(value_buf).y / 2) + 2), ImColor(0, 0, 0), value_buf, value_buf_end);
    window->DrawList->AddText(ImVec2(grab_bb.Max.x - (CalcTextSize(value_buf).x / 4), grab_bb.Min.y - (CalcTextSize(value_buf).y / 2) + 1), ImColor(255, 255, 255), value_buf, value_buf_end);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            window->DrawList->AddText(ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y - 1), text_col, is_open ? "-" : "+"); // RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y), text_col, is_open ? ImGuiDir_Down : ImGuiDir_Right, 1.0f);
    ...
    else
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            window->DrawList->AddText(ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.FontSize * 0.15f), ImColor(255, 255, 255), is_open ? "-" : "+"); // RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.FontSize * 0.15f), text_col, is_open ? ImGuiDir_Down : ImGuiDir_Right, 0.70f);
        if (g.LogEnabled)
